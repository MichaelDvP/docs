...
| `mixing_data` | data from The Mixing Module for each of the Thermometer's Heating Circuits and watercircuits. | `{"hc1":{"flowTemp":55,"pumpMod":"1","valveStatus":"1"}}` |
...
| `thermostat_cmd` | send a generic command to control the thermostat | JSON `{"cmd":<command><hc>,"data":<value>}` with `<command>` being `temp`, `mode`, `nighttemp`, `daytemp`, `holidaytemp`, `nofrosttemp`, `ecotemp`, `heattemp`, `offsettemp`, and `designtemp,`and `hc` optionally stating the heating circuit. `value` is a numeric floating point value | `{ "cmd":"daytemp2", "data": 20 }` |
